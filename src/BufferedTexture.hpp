...
template<std::size_t BUFFER_COUNT>
class BufferedTexture {
public:
    BufferedTexture(std::uint32_t width, std::uint32_t height, gs_color_format format = GS_BGRA, std::uint32_t flags = GS_DYNAMIC | GS_RENDER_TARGET)
        : width{width},
          height{height},
    ...
    gs_texture_t *getTexture() const noexcept
    {
        return texture.get();
    }
